...
export class ServerManagerService {
  ...
  private async tryBackendCommand(
    command: ServerCommand,
    ...
    try {
      ...
      const response = await isolatedFetch(
        `${backendUrl}/providers/${command.provider}/${command.action}`,
        {
          method: "POST",
          headers: {
      ...
      return {
        ...
        message:
          result.message || `${command.provider} ${command.action} completed`,
      };
    } catch (error) {
  ...
  private async directProviderCommand(
    command: ServerCommand,
  ): Promise<{ success: boolean; message: string }> {
  ...
  private async directOllamaCommand(
  ...
  private async directVLLMCommand(
    action: "start" | "stop" | "restart",
    modelName?: string,
  ): Promise<{ success: boolean; message: string }> {
    switch (action) {
      case "start":
        if (!modelName) {
          return {
            success: false,
            message:
              "Model name is required to start vLLM server.\n\n" +
              "To start vLLM:\n" +
        ...
        return {
          success: true,
          message:
            `To start vLLM with model "${modelName}":\n\n` +
  ...
  private async directLMStudioCommand(
    action: "start" | "stop" | "restart",
  ): Promise<{ success: boolean; message: string }> {
    switch (action) {
      case "start":
  ...
  async getProviderPaths(): Promise<{ [key: string]: string }> {
    ...
    return {
      ...
      lmstudio:
        "C:\\Users\\bcmad\\AppData\\Local\\Programs\\LM Studio\\LM Studio.exe",
