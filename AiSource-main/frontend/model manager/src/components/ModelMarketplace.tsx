...
const ModelMarketplace: React.FC<ModelMarketplaceProps> = ({
  ...
  const loadMarketplaceModels = async () => {
    ...
    if (
      modelsCache.current &&
      now - modelsCache.current.timestamp < CACHE_TTL
    ) {
    ...
    try {
      // Add cache-busting parameter and use isolated fetch
      timeoutId = setTimeout(() => controller.abort(), 10000); // 10 second timeout

      const response = await isolatedFetch(
        `/api/providers/marketplace/models?_=${now}`,
        {
      ...
      if (!response.ok) {
        // For backend unavailability (500/404), silently fall back to demo data
        if (response.status === 500 || response.status === 404) {
          console.debug(
            `Marketplace service unavailable (${response.status}), using demo data`,
          );
          // Set demo data without showing error to user
          const demoModels = generateDemoModels();
      ...
      const processedModels: MarketplaceModel[] = data.models.map(
        (model: any, index: number) => {
          const baseId =
            model.id ||
            `${model.provider || "model"}-${model.name || index}`
              .toLowerCase()
              .replace(/\s+/g, "-");
          ...
          return {
            ...
            lastUpdated: model.lastUpdated
              ...
              : new Date(),
            downloads:
              typeof model.downloads === "string"
                ? parseInt(model.downloads, 10)
                : model.downloads || 0,
            rating:
              typeof model.rating === "string"
                ? parseFloat(model.rating)
                : model.rating || 0,
      ...
      setUsingDemoData(false); // Clear demo data flag when real data is loaded
      // Update cache
      modelsCache.current = {
      ...
      console.debug("Marketplace fetch error:", err);

      // For expected network errors (backend not running), silently use demo data
      if (err instanceof Error) {
        if (
          ...
          // Silently fallback to demo data for expected connectivity issues
          console.debug("Backend unavailable, using demo marketplace data");
          const demoModels = generateDemoModels();
          setModels(demoModels);
          setUsingDemoData(true);
          modelsCache.current = {
        ...
        // Unknown error type - show generic message
        setError("Failed to load marketplace models");
      }

      // Fallback to demo data for any remaining errors
      console.info("Using demo marketplace data as fallback");
      const demoModels = generateDemoModels();
      setModels(demoModels);
      setUsingDemoData(true);
      modelsCache.current = {
      ...
      // Ensure timeout is always cleared and loading state is reset
      if (timeoutId) {
        clearTimeout(timeoutId);
      }
      setLoading(false);
    ...
    return [
      {
        ...
        description:
          "A 7 billion parameter language model fine-tuned for chat use cases",
        provider: "ollama",
        category: "chat",
        size: "3.8 GB",
        downloads: 125000,
      ...
      },
      {
        ...
        name: "Code Llama 13B",
        description:
          "Large language model specialized for code generation and understanding",
        provider: "ollama",
        category: "code",
        size: "7.3 GB",
        downloads: 89000,
      ...
      },
      {
        ...
        description:
          "High-performance 7B parameter model optimized for instruction following",
        provider: "vllm",
        category: "text",
        size: "13.5 GB",
        downloads: 156000,
  ...
  const handleInstallModel = async (model: MarketplaceModel) => {
    ...
    try {
      ...
      setModels((prev) =>
        prev.map((m) => (m.id === model.id ? { ...m, isInstalled: true } : m)),
      );
    } catch (error) {
            ...
            <div className="mt-4 p-4 bg-white dark:bg-gray-700 rounded-lg border border-gray-200 dark:border-gray-600">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <div className="space-y-1">
                    {["Apache 2.0", "MIT", "Custom", "Commercial"].map(
                      (license) => (
                ...
                <div>
                  ...
                  <div className="space-y-1">
                    {["GPU Support", "CPU Only", "Quantized", "Fine-tuned"].map(
                      (feature) => (
            ...
            <>
              ...
              {filteredModels.length > visibleCount && (
                <div className="flex justify-center mt-6">
                  ...
                  >
                    Load More Models ({filteredModels.length - visibleCount}{" "}
                    remaining)
                  </button>
                </div>
              )}
    ...
    <div className="bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg p-6 hover:shadow-lg transition-all duration-200 hover:border-blue-300 dark:hover:border-blue-600">
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1">
          ...
          <div className="flex items-center space-x-2 mb-2">
            <span
              className={`px-2 py-1 rounded-full text-xs font-medium ${getProviderColor(model.provider)}`}
            >
      ...
      {model.requirements && (
        <div className="mb-4 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
          ...
          <div className="text-xs text-gray-600 dark:text-gray-300">
            ...
            {model.requirements.recommendedRam &&
              ` (rec: ${model.requirements.recommendedRam})`}
