...
export const useSystemInfo = () => {
  ...
  const fetchSystemInfo = useCallback(async () => {
    ...
    try {
      ...
      const response = await isolatedFetch(`${backendUrl}/system/info`, {
      ...
      if (timeoutId) clearTimeout(timeoutId);
      console.warn(
        "Backend not available, attempting local system detection:",
        error,
      );
...
async function getRealSystemInfo(): Promise<SystemInfo> {
  ...
  if ("memory" in performance) {
    ...
    if (memory.totalJSHeapSize && memory.usedJSHeapSize) {
      const totalGB = (memory.totalJSHeapSize / (1024 * 1024 * 1024)).toFixed(
        1,
      );
...
async function checkProviderStatus() {
  const providers = {
    ollama: {
      installed: true,
      running: false,
      path: "C:\\Users\\bcmad\\AppData\\Local\\Programs\\Ollama\\ollama.exe",
    },
    lmstudio: {
      installed: true,
      running: false,
      path: "C:\\Users\\bcmad\\AppData\\Local\\Programs\\LM Studio\\LM Studio.exe",
    },
    vllm: { installed: true, running: false, path: "Ubuntu/WSL" },
  };

  // Check if services are actually running with shorter timeouts
  const createFetchWithTimeout = (url: string, timeout: number) => {
  ...
  const checks = await Promise.allSettled([
    createFetchWithTimeout("http://localhost:11434/api/tags", 2000),
    createFetchWithTimeout("http://localhost:1234/v1/models", 2000),
    createFetchWithTimeout("http://localhost:8000/health", 2000),
  ]);

  providers.ollama.running =
    checks[0].status === "fulfilled" && (checks[0].value as Response).ok;
  providers.lmstudio.running =
    checks[1].status === "fulfilled" && (checks[1].value as Response).ok;
  providers.vllm.running =
...
async function getCPUUsage(): Promise<number> {
  ...
  const usage = Math.min(
    100,
    Math.max(0, 100 - (iterations / baselineIterations) * 100),
  );
