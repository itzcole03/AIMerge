...
export const useProviders = () => {
  ...
  const loadProviderData = useCallback(async () => {
    ...
    try {
      ...
      const response = await isolatedFetch(`${backendUrl}/providers/status`, {
      ...
      if (response.ok) {
        ...
        console.debug(`Backend status request failed: ${response.status}`);
      }
    } catch (error) {
      console.debug(
        "Backend unavailable, using direct provider connections:",
        error,
      );
      // Don't reset cache here to avoid constant retries
    }
    ...
    const providerPromises = [
      ...
      (async () => {
        try {
          ...
          const backendRunning =
            backendStatus.providers?.ollama?.status === "running";
          const isHealthy =
            backendRunning || (await ollamaService.checkHealth());
          ...
          const [version, availableModels, runningModels] =
      ...
      (async () => {
        try {
          // Check backend status first, fallback to direct health check
          const backendRunning =
            backendStatus.providers?.vllm?.status === "running";
      ...
      (async () => {
        try {
          ...
          const backendRunning =
            backendStatus.providers?.lmstudio?.status === "running";

          if (!backendRunning) {
            console.log(
              "⚠️ LM Studio not detected by backend, marking as disconnected",
            );
            return {
              id: "lmstudio",
          ...
          console.log(
            "✅ LM Studio detected by backend, fetching models via proxy...",
          );
          ...
          if (models.status === "fulfilled") {
            ...
            console.log(
              `✅ Retrieved ${allModels.length} LM Studio models via backend proxy`,
            );
          } else {
